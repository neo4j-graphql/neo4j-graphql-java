...
class AggregationSelection(node: Node, selection: IResolveTree) {
    ...
    val count = selection.getFieldOfType(node.namings.aggregateTypeNames.selection, Constants.COUNT)

    val fieldSelection = selection.fieldsByTypeName[node.namings.aggregateTypeNames.selection]
    ...
    object Augmentation : AugmentationBase {
        ...
        fun addAggregationSelectionType(implementingType: ImplementingType, ctx: AugmentationContext): String {
            return ctx.getOrCreateObjectType(implementingType.namings.aggregateTypeNames.selection) { fields, _ ->
