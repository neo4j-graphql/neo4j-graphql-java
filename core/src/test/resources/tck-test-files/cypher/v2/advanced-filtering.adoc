...
----
{
  movies(where: {genres_ALL: {name_EQ: "some genre"}}) {
    actorCount
  }
}
----
...
----
{
  movies(where: {genres_SINGLE: {name_EQ: "some genre"}}) {
    actorCount
  }
}
----
...
----
{
  movies(where: {genres_SOME: {name_EQ: "some genre"}}) {
    actorCount
  }
}
----
...
----
{
  movies(where: {genresConnection_ALL: {node: {name_EQ: "some genre"}}}) {
    actorCount
  }
}
----
...
----
{
  movies(where: {genresConnection_SINGLE: {node: {name_EQ: "some genre"}}}) {
    actorCount
  }
}
----
...
----
MATCH (this:Movie)
WHERE single(ignore IN [(this)-[edge0:IN_GENRE]->(this0:Genre)
WHERE this0.name = $param0 | 1]
WHERE true)
RETURN this {
	.actorCount
...
----
{
  movies(where: {genresConnection_SOME: {node: {name_EQ: "some genre"}}}) {
    actorCount
  }
}
----
...
=== Node and relationship properties NONE
