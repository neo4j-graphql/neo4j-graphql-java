...
----
MATCH (this:Movie)
WITH this CALL apoc.util.validate(NOT (apoc.util.validatePredicate(NOT ($auth.isAuthenticated = true), '@neo4j/graphql/UNAUTHENTICATED', [0])), '@neo4j/graphql/FORBIDDEN', [0]) CALL apoc.util.validate(this.ratings IS NULL, 'Property %s cannot be NULL', ['ratings'])
