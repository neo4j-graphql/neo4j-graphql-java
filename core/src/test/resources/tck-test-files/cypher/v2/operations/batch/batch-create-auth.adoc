...
CALL {
	...
	CALL {
		...
		WITH count(this0_actors0_node_website_Website_unique) AS c CALL apoc.util.validate(NOT (c <= 1), '@neo4j/graphql/RELATIONSHIP-REQUIREDActor.website must be less than or equal to one', [0])
		RETURN c AS this0_actors0_node_website_Website_unique_ignored
	}
	WITH this0 CALL apoc.util.validate(NOT (any(r IN ['admin']
...
CALL {
	...
	CALL {
		...
		WITH count(this1_actors0_node_website_Website_unique) AS c CALL apoc.util.validate(NOT (c <= 1), '@neo4j/graphql/RELATIONSHIP-REQUIREDActor.website must be less than or equal to one', [0])
		RETURN c AS this1_actors0_node_website_Website_unique_ignored
	}
	WITH this1 CALL apoc.util.validate(NOT (any(r IN ['admin']
	WHERE any(rr IN $auth.roles
	WHERE rr = r))), '@neo4j/graphql/FORBIDDEN', [0])
	WITH this1
	CALL {
...
CALL {
	...
	WITH this2 CALL apoc.util.validate(NOT (any(r IN ['admin']
	WHERE any(rr IN $auth.roles
	WHERE rr = r))), '@neo4j/graphql/FORBIDDEN', [0])
	WITH this2
	CALL {
...
CALL {
	...
	CALL {
		...
		WITH this3, this3_actors_connect0_node CALL apoc.util.validate(NOT ((this3_actors_connect0_node.id IS NOT NULL
	...
	WITH this3 CALL apoc.util.validate(NOT (any(r IN ['admin']
	WHERE any(rr IN $auth.roles
	WHERE rr = r))), '@neo4j/graphql/FORBIDDEN', [0])
	WITH this3
	CALL {
...
CALL {
	...
	ON CREATE SET this4_actors_connectOrCreate0.name = $this4_actors_connectOrCreate_param1
		...
		WITH * CALL apoc.util.validate(NOT ((this4_actors_connectOrCreate0.id IS NOT NULL
			AND this4_actors_connectOrCreate0.id = $this4_actors_connectOrCreate0auth_param0)), '@neo4j/graphql/FORBIDDEN', [0])
		RETURN count(*) AS _
	}
	WITH this4 CALL apoc.util.validate(NOT (any(r IN ['admin']
	WHERE any(rr IN $auth.roles
	WHERE rr = r))), '@neo4j/graphql/FORBIDDEN', [0])
	WITH this4
	CALL {
		...
		WITH count(this4_website_Website_unique) AS c CALL apoc.util.validate(NOT (c <= 1), '@neo4j/graphql/RELATIONSHIP-REQUIREDMovie.website must be less than or equal to one', [0])
...
CALL {
	...
	CALL {
		...
		CALL {
			...
			WITH count(create_this6_website_Website_unique) AS c CALL apoc.util.validate(NOT (c <= 1), '@neo4j/graphql/RELATIONSHIP-REQUIREDActor.website must be less than or equal to one', [0])
	...
	CALL {
		...
		WITH count(create_this1_website_Website_unique) AS c CALL apoc.util.validate(NOT (c <= 1), '@neo4j/graphql/RELATIONSHIP-REQUIREDMovie.website must be less than or equal to one', [0])
