...
= Relationship nested operations -> Related to a union type -> Should not generate any nested operations if only CONNECT_OR_CREATE is specified and the related type does not have a unique field
