...
= @selectable -> relationships fields to an interface type -> Disable read on relationship field
...
type Actor @query(aggregate: true) {
  name: String!
  actedIn: [Production!]! @relationship(type: "ACTED_IN", direction: OUT) @selectable(onRead: false, onAggregate: true)
}
----
