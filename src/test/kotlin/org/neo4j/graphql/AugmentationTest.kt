...
class AugmentationTest {
    ...
    init {
        ...
        types.merge(SchemaParser().parse(directivesSdl))
    }
    ...
    fun testCrud() {
        ...
        createNodeMutation(ctx, nodeTypeFor("Person4"), types, projectionRepo).let {
            ...
            assertEquals("deletePerson4(id:ID!) : ID", it.delete)
            assertNull(it.query)
        }
        createNodeMutation(ctx, nodeTypeFor("Knows0"), types, projectionRepo).let {
            ...
            assertEquals("deleteKnows0(id:ID!) : ID", it.delete)
            assertNull(it.query)
        }
        createNodeMutation(ctx, nodeTypeFor("Knows1"), types, projectionRepo).let {
            ...
            assertNull(it.merge)
            assertNull(it.update)
            assertEquals("deleteKnows1(id:ID!) : ID", it.delete)
            assertNull(it.query)
        }
        createNodeMutation(ctx, nodeTypeFor("Knows4"), types, projectionRepo).let {
            ...
            assertEquals("mergeKnows4(_id:ID!, json:Object) : Knows4!", it.merge)
            assertEquals("updateKnows4(_id:ID!, json:Object) : Knows4!", it.update)
            assertEquals("deleteKnows4(_id:ID!) : ID", it.delete)
            assertNull(it.query)
        }
    }
    ...
    fun testQuery() {
        ...
        createNodeMutation(ctx, nodeTypeFor("Person1"), types, projectionRepo).let {
            assertEquals("person1(name:String, filter:_Person1Filter, orderBy:_Person1Ordering, first:Int, offset:Int) : [Person1!]!", it.query)
            assertNull(it.merge)
            assertNull(it.update)
            assertNull(it.delete)
            assertNull(it.create)
        ...
        assertEquals("person2(name:String, age:[Int], filter:_Person2Filter, orderBy:_Person2Ordering, first:Int, offset:Int) : [Person2!]!", createNodeMutation(ctx, nodeTypeFor("Person2"), types, projectionRepo).query)
        assertEquals("person3(name:String, filter:_Person3Filter, orderBy:_Person3Ordering, first:Int, offset:Int) : [Person3!]!", createNodeMutation(ctx, nodeTypeFor("Person3"), types, projectionRepo).query)
        assertEquals("person4(id:ID, name:String, filter:_Person4Filter, orderBy:_Person4Ordering, first:Int, offset:Int) : [Person4!]!", createNodeMutation(ctx, nodeTypeFor("Person4"), types, projectionRepo).query)

        assertEquals("knows4(_id:ID, json:Object, filter:_Knows4Filter, orderBy:_Knows4Ordering, first:Int, offset:Int) : [Knows4!]!", createNodeMutation(ctx, nodeTypeFor("Knows4"), types, projectionRepo).query)
    }
    ...
    fun testFilter() {
        ...
        assertEquals("input _Person2Filter { AND:[_Person2Filter!], OR:[_Person2Filter!], NOT:[_Person2Filter!], name:String, " +
                ...
                "name_not_ends_with:String, age:Int, age_not:Int, age_in:Int, age_not_in:Int, age_lt:Int, age_lte:Int, age_gt:Int, age_gte:Int }", createNodeMutation(ctx, nodeTypeFor("Person2"), types, projectionRepo).filterType)
