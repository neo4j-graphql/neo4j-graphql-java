...
class AugmentationTest {
    ...
    fun testCrud() {
        ...
        assertEquals("", createNodeMutation(ctx, types, typeFor("Person0")).create)

        // TODO a.s why merge and update are empty?
        createNodeMutation(ctx, types, typeFor("Person1")).let {
        ...
        assertEquals("createPerson2(name:String, age:[Int]) : Person2 ", createNodeMutation(ctx, types, typeFor("Person2")).create)
        assertEquals("createPerson3(name:String!) : Person3 ", createNodeMutation(ctx, types, typeFor("Person3")).create)
        createNodeMutation(ctx, types, typeFor("Person4")).let {
    ...
    fun testQuery() {
        ...
        assertEquals("", createNodeMutation(ctx, types, typeFor("Person0")).query)
        ...
        assertEquals("person2(name:String, age:[Int], _id: Int, filter:_Person2Filter, orderBy:_Person2Ordering, first:Int, offset:Int) : [Person2] ", createNodeMutation(ctx, types, typeFor("Person2")).query)
        assertEquals("person3(name:String, _id: Int, filter:_Person3Filter, orderBy:_Person3Ordering, first:Int, offset:Int) : [Person3] ", createNodeMutation(ctx, types, typeFor("Person3")).query)
        assertEquals("person4(id:ID, name:String, _id: Int, filter:_Person4Filter, orderBy:_Person4Ordering, first:Int, offset:Int) : [Person4] ", createNodeMutation(ctx, types, typeFor("Person4")).query)
    }

    @Test
    fun testFilter() {
        val ctx = Translator.Context(mutation = Translator.CRUDConfig(enabled = false), query = Translator.CRUDConfig(enabled = true, exclude = listOf("Person0")))
        assertEquals("", createNodeMutation(ctx, types, typeFor("Person0")).filterType)
        ...
        assertEquals("input _Person2Filter { AND:[_Person2Filter!], OR:[_Person2Filter!], NOT:[_Person2Filter!], name:String, " +
                ...
                "age_gt:[Int], age_gte:[Int] } ", createNodeMutation(ctx, types, typeFor("Person2")).filterType)
        assertEquals("input _Person3Filter { AND:[_Person3Filter!], OR:[_Person3Filter!], NOT:[_Person3Filter!], name:String, " +
                "name_not:String, name_in:String, name_not_in:String, name_lt:String, name_lte:String, name_gt:String, name_gte:String, " +
                "name_contains:String, name_not_contains:String, name_starts_with:String, name_not_starts_with:String, name_ends_with:String, " +
                "name_not_ends_with:String } ", createNodeMutation(ctx, types, typeFor("Person3")).filterType)
        assertEquals("input _Person4Filter { AND:[_Person4Filter!], OR:[_Person4Filter!], NOT:[_Person4Filter!], id:ID, id_not:ID, " +
                ...
                "name_not_ends_with:String } ", createNodeMutation(ctx, types, typeFor("Person4")).filterType)
    }

    @Test
    fun testInputTypes() {
        val ctx = Translator.Context(mutation = Translator.CRUDConfig(enabled = false), query = Translator.CRUDConfig(enabled = true, exclude = listOf("Person0")))
        assertEquals("", createNodeMutation(ctx, types, typeFor("Person0")).inputType)
        ...
        assertEquals("input _Person2Input { name:String, age:[Int] } ", createNodeMutation(ctx, types, typeFor("Person2")).inputType)
        assertEquals("input _Person3Input { name:String } ", createNodeMutation(ctx, types, typeFor("Person3")).inputType)
        assertEquals("input _Person4Input { id:ID, name:String } ", createNodeMutation(ctx, types, typeFor("Person4")).inputType)
    }

    @Test
    fun testOrderings() {
        val ctx = Translator.Context(mutation = Translator.CRUDConfig(enabled = false), query = Translator.CRUDConfig(enabled = true, exclude = listOf("Person0")))
        assertEquals("", createNodeMutation(ctx, types, typeFor("Person0")).ordering)
        ...
        assertEquals("enum _Person2Ordering { name_asc ,name_desc,age_asc ,age_desc } ", createNodeMutation(ctx, types, typeFor("Person2")).ordering)
        assertEquals("enum _Person3Ordering { name_asc ,name_desc } ", createNodeMutation(ctx, types, typeFor("Person3")).ordering)
        assertEquals("enum _Person4Ordering { id_asc ,id_desc,name_asc ,name_desc } ", createNodeMutation(ctx, types, typeFor("Person4")).ordering)
    }
    ...
    fun testMutations() {
        ...
        assertEquals("", createNodeMutation(ctx, types, typeFor("Person0")).ordering)
        assertEquals("", createNodeMutation(ctx, types, typeFor("Person1")).filterType)
        assertEquals("", createNodeMutation(ctx, types, typeFor("Person2")).query)
        assertEquals("", createNodeMutation(ctx, types, typeFor("Person3")).inputType)
