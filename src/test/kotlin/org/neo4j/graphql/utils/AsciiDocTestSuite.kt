 ...
 * @blocks the markers for on test case
 * @globalBlocks the markers for global blocks
 */
open class AsciiDocTestSuite(
        private val fileName: String,
        private val blocks: List<String> = emptyList(),
        private val globalBlocks: List<String> = listOf(SCHEMA_MARKER)) {
    ...
    private inner class FileParser {
        ...
        fun parse(): Stream<DynamicNode> {
            ...
            loop@ for ((lineNr, line) in lines.withIndex()) {
                ...
                when {
                    globalBlocks.contains(line) -> currentBlock = startBlock(line, lineNr, globalCodeBlocks)
                    blocks.contains(line) -> currentBlock = startBlock(line, lineNr, codeBlocksOfTest)
                    line == "----" -> {
                        inside = !inside
                        if (inside) {
    ...
    private fun printAdjustedTestFile() {
        knownBlocks.sortWith(compareByDescending<ParsedBlock> { it.start }
            .thenByDescending { blocks.indexOf(it.marker) })
    ...
    protected fun getOrCreateBlock(codeBlocks: Map<String, ParsedBlock>, marker: String, headline: String): ParsedBlock? {
        ...
        if (block == null && GENERATE_TEST_FILE_DIFF) {
            val insertPoints = blocks.indexOf(marker).let { blocks.subList(0, it).asReversed() }
