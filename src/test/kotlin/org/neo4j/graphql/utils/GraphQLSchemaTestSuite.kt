...
class GraphQLSchemaTestSuite(fileName: String) : AsciiDocTestSuite() {
    ...
    class TestRun(
        ...
        fun run() {
            println(title)
            try {
                val augmentedSchema = SchemaBuilder.buildSchema(suite.schema, context)
                val schemaParser = SchemaParser()
                val reg = schemaParser.parse(targetSchema)
                val schemaGenerator = SchemaGenerator()
                val expected = schemaGenerator.makeExecutableSchema(reg, RuntimeWiring.newRuntimeWiring()
                    .scalar(ObjectScalar())
                    .build())

                val diffSet = DiffSet.diffSet(expected, augmentedSchema)
                val print = PrintStreamReporter(System.out)
                val capture = CapturingReporter()
                SchemaDiff(SchemaDiff.Options.defaultOptions().enforceDirectives())
                    .diffSchema(diffSet, ChainedReporter(listOf(print, capture)))
